<template>
  <div class="container">
    <form @submit.prevent="handleChangePassword" class="form-signin">
      ...
      <div v-if="successMessage" class="text-success" style="color: green;">{{ successMessage }}</div>
      <div v-if="errors.generalMessage" class="text-success" style="color: green;">{{ errors.generalMessage }}</div>
      ...
      <p>
        ...
        <input
          ...
          required
        />
        <span v-if="errors.currentPassword" style="color: red">{{ errors.currentPassword }}</span>
      </p>
      <p>
        ...
        <input
          ...
          required
        />
        <span v-if="errors.newPassword" style="color: red">{{ errors.newPassword }}</span>
      </p>
      <p>
        ...
        <input
          ...
          required
        />
        <span v-if="errors.newPasswordConfirmation" style="color: red">{{ errors.newPasswordConfirmation }}</span>
      </p>
      ...
      try {
        if (isGraphQL) {
          ...
          if (!response.ok || responseData.errors) {
            throw new Error(
              responseData.errors?.[0]?.message || "An error occurred while changing the password."
            );
          }
          ...
          const response = await fetch(`${this.BASE_URL}/api/users/change-password`, {
            method: "PUT",
            headers,
          ...
          if (!response.ok) {
            ...
            } else if (errorMessage.includes("Password must be strong (at least 6 characters, one uppercase, one lowercase, one digit, and one special character)")) {
